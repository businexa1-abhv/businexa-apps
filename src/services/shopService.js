...
async function updateShopById(shopId, ownerId, updates) {
  const shop = await Shop.findById(shopId);
  if (!shop) return { error: 'not_found' };
  if (String(shop.ownerId) !== String(ownerId)) return { error: 'forbidden' };
...
async function deleteShopById(shopId, ownerId) {
  const shop = await Shop.findById(shopId);
  if (!shop) return { error: 'not_found' };
  if (String(shop.ownerId) !== String(ownerId)) return { error: 'forbidden' };
