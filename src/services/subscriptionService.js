...
async function createRazorpayOrder({ planId, shopId, userId }) {
  const plan = getPlanById(planId);
  if (!plan) return { error: 'invalid_plan' };

  const shop = await Shop.findById(shopId);
  if (!shop || String(shop.ownerId) !== String(userId)) return { error: 'forbidden' };
...
async function cancelSubscriptionById(subscriptionId, userId) {
