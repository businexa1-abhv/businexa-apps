...
async function createOrder(req, res, next) {
  try {
    ...
    if (result.error === 'shop_not_found') throw new AppError('Shop not found', 404);
...
async function listSubscriptions(req, res, next) {
  try {
    if (req.dbUser.role === 'admin') {
      ...
      const subscriptions = await subscriptionService.listAllSubscriptions(limit);
...
async function shopSubscription(req, res, next) {
  try {
    ...
    const isAdmin = req.dbUser.role === 'admin';
    if (!isAdmin && String(shop.ownerId) !== String(req.dbUser._id)) throw new AppError('Forbidden', 403);
